...
Nuts will serve NuGet packages on `http://download.myapp.com/update/RELEASES`.

Your application just need to configurer `Update.exe` or `Squirrel.Windows` to use `http://download.myapp.com/update` as a feed url (:warning: without query parameters).
