...
import { mergeReleaseNotes } from "./utils/notes.js";
import {
  platforms,
  detectPlatform,
  resolveForVersion,
} from "./utils/platforms.js";
import { parseRELEASES, generateRELEASES } from "./utils/win-releases.js";
...
export class Pecans {
  ...
  async handleDownload(req, res, next) {
    ...
    try {
      ...
      if (filename) {
        ...
        asset = resolveForVersion(version, platform, {
  ...
  async handleUpdate(req, res, next) {
    try {
      ...
      const platform = detectPlatform(req.params.platform);
      ...
      const releaseNotes = mergeReleaseNotes(notesSlice, { includeTag: false });
  ...
  async handleUpdateWin(req, res, next) {
    try {
      await this.init();
      const fullUrl = this.getFullUrl(req);
      const platform = detectPlatform(req.params.platform) || "win_32";
      const channel = req.params.channel || "*";
      const tag = req.params.version;
      ...
      releases = releases
        ...
        .map((entry) => {
          const gitFilePath =
            channel === "*" ? "../../../../" : "../../../../../../";
          entry.filename = urljoin.default(
            fullUrl,
            gitFilePath,
  ...
  async handleServeNotes(req, res, next) {
    try {
      ...
      res.format({
        "text/plain": function () {
          res.send(mergeReleaseNotes(versions));
        },
        "application/json": function () {
          res.send({
            notes: mergeReleaseNotes(versions, { includeTag: false }),
            pub_date: latest.published_at.toISOString(),
          });
        },
        default: function () {
